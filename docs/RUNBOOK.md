...
| **GitHub repo** | Remote code (tuỳ chọn); deploy do dev chạy CLI sau khi merge/pull |
...
2. **Maintainer code:** GitHub / team dev — pull code, build, `firebase deploy`.
...
```bash
cd /path/to/repo
cp .env.example .env   # nếu chưa có — điền đủ NEXT_PUBLIC_*
...
Trước deploy nên: `npm run lint` và `npm run build` để bắt lỗi sớm.

### Rollback nhanh

1. **Git:** checkout commit / tag ổn định, rồi `firebase deploy --only hosting` lại từ máy tin cậy.
...
- **Preview URL:** có thể dùng [Firebase Hosting preview channels](https://firebase.google.com/docs/hosting/test-preview-deploy) từ CLI (`firebase hosting:channel:deploy`) khi cần — không cấu hình sẵn trong repo.
- **Staging vs production:** `.firebaserc` có thể trỏ một project; production riêng: thêm alias / project thứ hai và chọn đúng alias trước khi `deploy`.
